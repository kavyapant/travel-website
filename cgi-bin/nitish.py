...
city=form.getvalue('abc')
query=form.getvalue('filter')
...
qaz="select * from amsr where location=\""+city+"\" order by "+query+" "+order
cur=conn.execute(qaz)
#cur=conn.execute('select * from amsr where location=? limit 10',(city,))
...
k=0
print("<h1 align=center>Restaurants in %s</h1>"%(city))
print("Filter:<form action=nitish.py><input type=hidden name=abc value=%s>"%(city))
...
for i in cur:
    ...
    co=co+1       
    data_uri = base64.b64encode(open('restimages/'+str(k)+'.jpg', 'rb').read()).decode('utf-8')
    img_tag = '<img src="data:image/jpg;base64,{0}">'.format(data_uri)
    print(img_tag)
    print("</td><th>")
    print("%s </th></tr><tr>" % (i[1]))
    print("<td>Cost: Rs. %s</td>"%i[5])
  #  print("<td rowspan=2><form action=/hotels/hotels.html>")
   # print("<input type=Submit class=button value=Book name=%s></form></td>"%(i[0]))
